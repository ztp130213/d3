...
该模块为二维多边形提供了几个基本的几何操作。每个多边形都表示为一个由二元数组成的数组[[ x1，y1 ]，[ x2，y2 ]，...]，并且可以是封闭的（其中起点和终点是相同的）或者开放的（当他们起点和终点不闭合）。典型的多边形按照逆时针顺序，假设一个坐标系，其中原点⟨0,0⟩位于左上角
...
使用[]Andrew的单调链算法](http://en.wikibooks.org/wiki/Algorithm_Implementation/Geometry/Convex_hull/Monotone_chain)返回指定点的[凸包](https://en.wikipedia.org/wiki/Convex_hull)。返回的块表示为一个数组，其中包含按逆时针顺序排列的输入点子集。如果点少于三个元素，则返回null。
...
当且仅当定的点是[指定的内部多边形](https://www.ecse.rpi.edu/Homepages/wrf/Research/Short_Notes/pnpoly.html)则返回true。
